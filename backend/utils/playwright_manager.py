...
def jsx_to_html(jsx_string, context={}):
    # Use Babel to transform JSX, then evaluate with context
    js_code = f"""
    const babel = require('@babel/core');
    const result = babel.transform(`{jsx_string}`, {{
        presets: ['@babel/preset-react']
    }});
    // Evaluate the result with your context
    eval(result.code);
    """
    
    ctx = execjs.compile(js_code)
    return ctx.call("render", context)